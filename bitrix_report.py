...
def count_leads_between(cfg: dict, date_from: str, date_to: str) -> int:
    """Подсчёт лидов за период в отслеживаемых статусах — единый срез с отчётом:
    шапка, план и прогноз показывают те же цифры, что и тело отчёта."""
    webhook = cfg["bitrix_webhook"]
    flt = {">=DATE_CREATE": date_from, "<DATE_CREATE": date_to}
    if cfg.get("statuses"):
        flt["STATUS_ID"] = cfg["statuses"]
...
HELP_ADMIN = (
    ...
    "брали, попытки оплатить) и сразу после него — прогноз на сегодня\n"
    "• Каждый понедельник — недельная сводка: цель, реальность, динамика, "
    "прогноз и «Задачи и фокус»\n"
    "• 1-го числа — отчёт за месяц\n"
    "• Если что-то ломается — бот сам напишет ⚠️ с причиной. Дублей нет: "
    "помнит, что уже отправлял.\n"
    "\n"
    "<b>ПЛАН — задаётся в этом чате</b>\n"
    "/plan 50 — цель: 50 лидов в день\n"
    "/plan — прогресс · /plan удалить — сброс\n"
    "«Темп 13/день» = все лиды с 1-го числа ÷ прошедшие дни. Прогресс виден "
    ...
    "\n"
    "<b>КАК ЧТО СЧИТАЕТСЯ</b>\n"
    "• Отчёты — лиды в статусах: Новый, Прогрев, Попытка оплатить курс, Диалог "
    "с куратором, Диагностика, Конвертирован\n"
    "• Динамика и прогнозы — по ВСЕМ созданным лидам: старые лиды меняют статус, "
    "и тренд по статусам врал бы\n"
    "• Прогноз = линейный тренд + сезон дня недели; бот помнит свои прогнозы и "
    "честно сверяет с фактом («прогноз был 12 → факт 10»)\n"
    "• Страна — по коду телефона (+7 6xx/7xx = Казахстан)\n"
    "• Уроки — из поля «Продукт»: хвосты «- 1x0 = 0» и слова «Бесплатно / "
    "Бесплатная часть / Тариф…» отрезаются, один курс склеивается из вариантов\n"
    "• «Попытка оплатить» — уникальные люди по телефону: сколько дублей ни "
    "наделай, попытка одна\n"
    "\n"
...
HELP_WORK = (
    ...
    "• Понедельник — недельная сводка, прогноз и «Задачи и фокус»\n"
    "• 1-го числа — отчёт за месяц\n"
    "\n"
    "<b>ОСОБЕННОСТИ ЭТОГО ЧАТА</b>\n"
    "• Строк «(без метки)» нет — только реальные источники\n"
    "• Бесплатные уроки и попытки оплатить сгруппированы по источникам рекламы\n"
    "• Лид без метки приписывается источнику по телефону: ищем номер в истории "
    "за 90 дней и берём последнюю рекламу, по которой человек приходил\n"
    "• У попыток оплатить — первый контакт: дата и метка первого касания + "
    "сколько дней человек шёл до оплаты\n"
    "• Динамика в прогнозах — только лиды с метками (безымянных ищем по телефону)\n"
    "\n"
    "<b>ПЛАН</b>\n"
    ...
    "\n"
    "<b>КАК ЧТО СЧИТАЕТСЯ</b>\n"
    "• Статусы: Новый, Прогрев, Попытка оплатить курс, Диалог с куратором, "
    "Диагностика, Конвертирован\n"
    "• Прогноз = тренд + сезон дня недели, на следующий день/неделю бот сверяет "
    "его с фактом\n"
    "• «Попытка оплатить» — один человек (по телефону) = одна попытка\n"
...
def process_command(cfg: dict, text: str, clean: bool = False, no_utm: bool = False,
                    plan: dict = None, admin: bool = False):
